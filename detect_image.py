...
# load the list of categories in the COCO dataset and then generate a set
# of bounding box colors for each class
CLASSES = pickle.loads(open(args["labels"], "rb").read())
COLORS = np.random.uniform(0, 255, sizes=(len(CLASSES), 3))
...
# convert the image from BGR to RGB channel ordering and change the image from
# channels last to channels first ordering
image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
image = image.transpose((2, 0, 1))

# add the batch dimesion, scale the raw pixel intensities to the range [0, 1]
