...
for e in range(0, EPOCHS):
    ...
    totalTrainLoss = 0
    totalValLoss = 0
    # initialize the number of correct predictions in the training and validationn step
    trainCorrect = 0
    valCorrect = 0
    ...
    for (x, y) in trainDataLoader:
        ...
        loss.backward()
        opt.step()
        # add the loss to the total training loss so far and the number of correct predictions
        totalTrainLoss += loss
    ...
    print("Train loss: {:.6f}, Train accuracy: {:.4}".format(avgTrainLoss, trainCorrect))
...
with torch.no_grad():
    ...
    for (x, y) in testDataLoader:
        x = x.to(device)
        pred = model(x)
        preds.append(pred.argmax(axis=1).cpu().numpy())
