...
# create training and testing splots, and convert them to PyTorch tensor
...
for epoch in range(0, EPOCHS):
    # initialize tracker variable and set pur model to trainable
    ...
    # initilize tracker variable for testing, then set our model to evaluation mode
    testLoss = 0
    testAcc = 0 
    samples = 0
    mlp.eval()
    ...
    with torch.no_grad():
        for (batchX, batchY) in next_batch(testX, testY, BATCH_SIZE):
            ...
            predictions = mlp(batchX)
            loss = lossFunc(predictions, batchY.long())
            # update test loss, accuracy, and the number of samples
