...
# load the network weights from disk, flash it to the current device, and set it to evaluation mode
print("[INFO] loading {}...".format(args["model"]))
model = MODELS[args["model"]].to(config.DEVICE)
model.eval()

# load the image from disk, clone it, and preprocess ot
...
# loop over the predictions and display the rank-5 predictions and corresponding 
# probabilities to our terminal
for (i, idx) in enumerate(sortedProba[0, :5]):
    print("{}. {}: {:.2f}%".format(i, imagenetLabels[idx.item()].strip(),
        probabilities[0, idx.item()] * 100))

# draw the top prediction on the image and display the image to screen
